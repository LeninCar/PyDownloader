...
def obtener_fecha_publicacion(url_video):
    comando_fecha = subprocess.check_output(['yt-dlp','--no-warnings', '--no-warnings','--get-filename', '-o' ,'%(upload_date)s', url_video], text=True,timeout=60)
...
def obtener_nombres_urls(url_canal):
    ...
    salida = subprocess.check_output(comando, text=True, timeout=60).strip().split('\n')
...
def procesar_videos(ih, nh, nombres_videos, urls_videos, directorio_destino):
    for i in range(ih, len(nombres_videos), nh):
        ...
        extension = subprocess.check_output(['yt-dlp', '--no-warnings', '--flat-playlist', '--get-filename', '-o', '%(ext)s', url_video], text=True, timeout=60).strip()
...
if __name__ == '__main__':
    ...
    for i in range(NH):
        # Crear un proceso con la función 'procesar_videos'
        proceso = multiprocessing.Process(target=procesar_videos, args=(i, NH, nombres_videos, urls_videos, directorio_destino))        # Comenzar el proceso
        # Comenzar el proceso
        proceso.start()
        # Adicionar el proceso a la lista de procesos
